...
const shouldOpenMiniplayerOnNavigate = globalThis.YtAmp.shouldOpenMiniplayerOnNavigate;

// A search, or any navigation that is not a link click.
function navigateState(changes) {
  return Object.assign({
    enabled: true,
    onWatchPage: true,
    sameHost: true,
...
test('navigate: all conditions pass', () => {
  assert.strictEqual(shouldOpenMiniplayerOnNavigate(navigateState()), true);
});

test('navigate: the toggle is off', () => {
  assert.strictEqual(
    shouldOpenMiniplayerOnNavigate(navigateState({ enabled: false })), false);
});

test('navigate: the page is not a watch page', () => {
  assert.strictEqual(
    shouldOpenMiniplayerOnNavigate(navigateState({ onWatchPage: false })), false);
});

test('navigate: the destination is another host', () => {
  assert.strictEqual(
    shouldOpenMiniplayerOnNavigate(navigateState({ sameHost: false })), false);
});

test('navigate: the destination is another watch page', () => {
  assert.strictEqual(shouldOpenMiniplayerOnNavigate(
    navigateState({ destinationPath: '/watch' })), false);
});

test('navigate: a queue or a playlist is active', () => {
  assert.strictEqual(
    shouldOpenMiniplayerOnNavigate(navigateState({ queueOpen: true })), false);
});

test('navigate: the miniplayer is open', () => {
  assert.strictEqual(
    shouldOpenMiniplayerOnNavigate(navigateState({ miniplayerOpen: true })), false);
});

// The i key makes its own push navigation. One press is enough.
test('navigate: the extension acted a moment ago', () => {
  assert.strictEqual(
    shouldOpenMiniplayerOnNavigate(navigateState({ recentlyActed: true })), false);
});

test('navigate: the state is missing', () => {
  assert.strictEqual(shouldOpenMiniplayerOnNavigate(undefined), false);
});
